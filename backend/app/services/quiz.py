...
def _load_or_create_attempt(db, user_id: str, user_email: str, quiz_id: str) -> dict:
    ...
    # TODO(question-reuse): each attempt draws independently from the whole pool,
    # so now that a participant runs the base quiz plus every variant, the same
    ...
    #
    # Fix by excluding question_ids already present in this user's other
    # quiz_attempts before slicing, e.g.:
    #
    ...
    #
    # That needs the question bank to hold at least
    # MAX_QUIZ_QUESTIONS * (1 + number of variants) items — 40 at present — so it
    # also needs a clear error (or a smaller MAX_QUIZ_QUESTIONS) when the pool is
    # too small, rather than silently serving short quizzes.
...
def _mark_quiz_completed(db, doc: dict) -> None:
    ...
    # assigned step_order. The old base-vs-variant branching lived here and in
    # three other places; services/study_flow owns all of it now, so a quiz_id
    # that isn't part of this user's flow (admin test runs) is simply a no-op.
